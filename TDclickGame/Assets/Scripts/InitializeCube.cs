...
public class InitializeCube : MonoBehaviour
{
    ...
    protected bool RoadCheck(int col, int row, CreateMap map)
    {
        ...
        if (map.GetTile(col + 1, row, out tile))
        {
            ...
            if (up)
            {
                up = tile.GetComponent<InitializeCube>().RoadCheckUp(col + 1, row, map, gameObject);
            }
        }
        if (map.GetTile(col - 1, row, out tile))
        {
            ...
            if (down)
            {
                down = tile.GetComponent<InitializeCube>().RoadCheckDown(col - 1, row, map, gameObject);
            }
        }
        if (map.GetTile(col, row - 1, out tile))
        {
            ...
            if (left)
            {
                left = tile.GetComponent<InitializeCube>().RoadCheckLeft(col, row - 1, map, gameObject);
            }
        }
        if (map.GetTile(col, row + 1, out tile))
        {
            ...
            if (right)
            {
                right = tile.GetComponent<InitializeCube>().RoadCheckLeft(col, row + 1, map, gameObject);
            }
        }
    ...
    protected bool RoadCheckUp(int col, int row, CreateMap map, GameObject lastPath)
    {
        ...
        if (type == CubeType.Road)
        {
            ...
            if (map.GetTile(col, row + 1, out tile))
            {
                right = tile.GetComponent<InitializeCube>().RoadCheckDown(col, row + 1, map, gameObject);
            }
            if (map.GetTile(col, row - 1, out tile))
            {
                left = tile.GetComponent<InitializeCube>().RoadCheckLeft(col, row - 1, map, gameObject);
    ...
    protected bool RoadCheckDown(int col, int row, CreateMap map, GameObject lastPath)
    {
        ...
        if (type == CubeType.Road)
        {
            ...
            if (map.GetTile(col, row + 1, out tile))
            {
                right = tile.GetComponent<InitializeCube>().RoadCheckUp(col, row + 1, map, gameObject);
            }
            if (map.GetTile(col - 1, row, out tile))
            {
                down = tile.GetComponent<InitializeCube>().RoadCheckDown(col - 1, row, map, gameObject);
